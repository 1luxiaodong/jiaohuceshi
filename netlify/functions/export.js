...
exports.handler = async (event) => {
  ...
  try {
    ...
    const detail = sessions.map(r => ({
      ...
      '衣服点击次数': r.image_clicks || 0,
      '购物车点击次数': r.cart_clicks || 0,
      '记录时间': r.created_at
    }));
    ...
    const summary = Object.values(grouped).map(g => ({
      ...
      '平均停留时长(秒)': Math.round(g.visits.reduce((a, b) => a + b, 0) / g.visits.length * 100) / 100,
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(detail), '详细记录');
